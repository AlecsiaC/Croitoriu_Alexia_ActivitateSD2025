...
struct Masina* copiazaPrimeleNElemente(struct Masina* vector, int nrElemente, int *nrElementeCopiate) {
	...
	vectorNou = NULL;
 	if (vector != NULL && (*nrElementeCopiate) > 0) {
		...
		vectorNou = malloc(sizeof(struct Masina) * (*nrElementeCopiate)); //initializare
...
void dezalocare(struct Masina** vector, int* nrElemente) {
	for (int i = 0; i < (*nrElemente); i++) {
		if ((*vector)[i].model != NULL) {
			free((*vector)[i].model); //dereferentiere + deplasare + dereferentiere + accesare -> pointer la pointer la pointer
		}
	}
	free(*vector);
	*vector = NULL;
