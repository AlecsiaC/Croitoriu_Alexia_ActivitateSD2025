...
struct Masina {
	...
	char* model;
	float pret;
	char tipMotor; //codul ASCII al caracterului
};
...
struct Masina initializare(int id, int nrKm, const char* model, float pret, char tipMotor) {
	...
	m.id = id;
	m.nrKm = nrKm;
	m.model = (char*)malloc(sizeof(model) + 1); //pointer la void = pointer general, stramosul lui template
	//sizeof = cati octeti ocupa un param.
